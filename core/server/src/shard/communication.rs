...
impl IggyShard {
    ...
    pub fn find_shard_table_record(&self, namespace: &IggyNamespace) -> Option<ShardLocation> {
        self.shards_table.get(namespace).map(|entry| *entry)
    }

    pub fn remove_shard_table_record(&self, namespace: &IggyNamespace) -> ShardLocation {
        self.shards_table
            .remove(namespace)
    ...
    pub fn insert_shard_table_record(&self, ns: IggyNamespace, location: ShardLocation) {
